from CHIDS.tools.baseline import Baseline
import typer
...
def baseline(input_dir_path: str = typer.Option(..., "--td"), output_dir_name: str = typer.Option(..., "--od")):
    ...
    files = prepare_files(input_dir_path)
    seen_syscalls, seen_args, max_freq, model, thresh_list = Baseline(files).get_training_elements()
    ...
    output_table.add_row(str(len(files)), str(seen_syscalls)[1:-1], str(seen_args)[1:-1], str(thresh_list)[1:-1])
...
def _get_evaluation_results(_dir, seen_syscalls, seen_args, freq_max, trained_model, thresh_list):
    ...
    files = prepare_files(_dir)
    baseline_obj = Baseline(files)
    scaps_dfs = baseline_obj._files_to_dfs()
